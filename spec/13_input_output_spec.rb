...
describe NumberGame do
  ...
  describe '#game_over?' do
    ...
    context 'when user guess is not correct' do
      subject(:game_continue) { described_class.new(9, '1') }
      # remove the 'x' before running this test
  ...
  describe '#final_message' do
    ...
    context 'when count is 4 and over' do
      # remove the 'x' before running this test
      subject(:game) { described_class.new(7, '7', 9) }
      it 'outputs correct phrase' do
        game_over_phrase = "That was hard. It took you 9 guesses!\n"
        expect { game.final_message }.to output(game_over_phrase).to_stdout
      end
    end
  end
end
